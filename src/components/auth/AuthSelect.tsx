  ...
  ({ label, options, className = "", error, ...props }, ref) => (
    <div className="w-full space-y-2">
      ...
      <div className="relative">
        <select
          ref={ref}
          className={`w-full px-4 py-3 rounded-lg bg-white/30 border ${
            error ? "border-red-400" : "border-white/20"
          } text-white focus:outline-none focus:ring-2 focus:ring-white/50 focus:bg-white/40 transition-all backdrop-blur-sm appearance-none ${className}`}
          {...props}
        >
      ...
      {error && (
        <p className="text-red-200 text-xs ml-1 font-medium bg-red-500/20 p-1 rounded w-fit">
          {error}
        </p>
      )}
    </div>
